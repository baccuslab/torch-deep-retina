...
def latenc(mdls, intensities, baseline):
    rates = []
    for i in tqdm(intensities):
        X = s.flash(2, 40, 70, intensity=i, baseline=baseline)
...
def main():
    ...
    ts = np.linspace(0, 200, 1000)
    baseline = 0
    intensities = np.linspace(baseline, -3, 21)

    wn = latenc(whits, intensities,baseline)
    ns = latenc(nats, intensities,baseline)

    #intensities = -(baseline-intensities)/baseline*100
    wn_intens = intensities
    ns_intens = intensities

    #wn_mean = np.mean([m.norm_stats['mean'] for m in whits])
    #wn_std = np.mean([m.norm_stats['std'] for m in whits])
    #ns_mean = np.mean([m.norm_stats['mean'] for m in nats])
    #ns_std = np.mean([m.norm_stats['std'] for m in nats])

    #wn_intens = (intensities*wn_std)+wn_mean
    #wn_intens = (wn_intens-wn_mean)/wn_mean*100
    #ns_intens = (intensities*ns_std)+ns_mean
    #ns_intens = (ns_intens-ns_mean)/ns_mean*100
    ...
    subp = 131
    ax1 = fig.add_subplot(subp)
    wn_resps = np.asarray(all_wnresps)[0]
    colors = cm.gray_r(np.linspace(0, 1, len(wn_resps)))
    for r, c in zip(wn_resps, colors):
        ax1.plot(ts[:endx], r[:endx], '-', color=c)
    #ax1.set_xlabel('Time (s)',fontsize=40)
    ...
    subp = 132
    ax2 = fig.add_subplot(subp)
    ns_resps = np.asarray(all_nsresps)[0]
    colors = cm.Reds(np.linspace(0, 1, len(ns_resps)))
    for r, c in zip(ns_resps, colors):
        ax2.plot(ts[:endx], r[:endx], '-', color=c)
    #ax2.set_xlabel('Time (s)',fontsize=40)
    #ax2.set_ylabel('Response (Hz)',fontsize=40)
    ...
    ax.plot(-wn_intens, wn_lats, 'o', color=whit_color)
    ax.plot(-ns_intens, ns_lats, 'o', color=nat_color)
    ...
    plt.locator_params(nbins=3)
    plt.tight_layout()
    plt.savefig("lat_encs/"+prename+'latency_encoding_latencies.png')
    plt.savefig("lat_encs/"+prename+'latency_encoding_latencies.pdf')
