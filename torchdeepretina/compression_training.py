...
class Trainer:
    ...
    def zero_chans(self, model, chan_dict):
        """
        ...
        for layer in chan_dict.keys():
            idx = int(layer.split(".")[-1]) # Assumes layer name is of form 'sequential.<idx>'
            for chan in chan_dict[layer]:
                shape = model.sequential[idx].weight.data[chan].shape
                model.sequential[idx].weight.data[chan] = torch.zeros(*shape[1:]).to(DEVICE)
    ...
    def train(self, hyps, model_hyps, device, verbose=False):
        ...
        # Make the layer channel sets for dropping channels
        zero_dict = {d:set() for d in hyps['intg_layers']}
        intg_idx = 0

        # Training
        epoch = -1
        prev_acc = -1
        stop_training_loop = False
        while not stop_training_loop:
            epoch += 1
            ...
            # Train Loop
            for i, (x,label) in enumerate(data_distr.train_sample(batch_size=hyps['batch_size'])):
                optimizer.zero_grad()
                ...
                loss.backward()
                optimizer.step()
                self.zero_chans(model, zero_dict)
