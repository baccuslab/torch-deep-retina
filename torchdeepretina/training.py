    ...
    def retinotopic_loop(self, hyps, verbose=False):
        ...
        while not stop_training:
            ...
            for i,(x,label) in enumerate(data_distr.train_sample()):
                ...
                if 'semantic_loss_scale' not in hyps:
                    scale = 10
                else:
                    scale = hyps['semantic_loss_scale']

                one_hot_params = model.sequential[-1]
                one_hot_loss = semantic_loss(one_hot_params.prob) * scale
...
def train_ln(X, y, rf_center, cutout_size,verbose=True):
    ...
    sta, norm_stats, _ = tdrutils.revcor(X, y, batch_size=500,
                                          ret_norm_stats=True)
