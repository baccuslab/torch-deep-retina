...
class KineticsChannelModelLayerNorm(nn.Module):
    def __init__(self, drop_p=0, scale_kinet=True, recur_seq_len=8, n_units=5, 
        ...
        super(KineticsChannelModelLayerNorm, self).__init__()
        ...
        self.h_shapes.append((self.chans[0], shape[0]*shape[1]))
        modules.append(Flatten())
        modules.append(nn.LayerNorm(self.chans[0] * shape[0] * shape[1]))   
        #modules.append(Add(-1.5))
        modules.append(nn.Softplus())
        max_clamp = 10
        modules.append(Clamp(0,max_clamp))
        ...
        self.shapes.append(tuple(shape))
        modules.append(Flatten())
        modules.append(nn.LayerNorm(self.chans[1] * shape[0] * shape[1]))
        modules.append(nn.ReLU())
        self.amacrine = nn.Sequential(*modules)
        ...
        modules.append(nn.Linear(self.chans[1]*shape[0]*shape[1], 
                                 self.n_units, bias=self.linear_bias))
        modules.append(nn.LayerNorm(self.n_units))
        modules.append(nn.Softplus())
        self.ganglion = nn.Sequential(*modules)
...
class KineticsChannelModelNoBatchnorm(nn.Module):
    def __init__(self, bnorm=True, drop_p=0, scale_kinet=False, recur_seq_len=5, n_units=5, 
                 noise=.05, bias=True, linear_bias=False, chans=[8,8], softplus=True, 
                 inference_exp=False, img_shape=(40,50,50), ksizes=(15,11), centers=None):
        super(KineticsChannelModelNoBatchnorm, self).__init__()
...
class KineticsChannelModelFilterNoClamp(nn.Module):
    def __init__(self, bnorm=True, drop_p=0, scale_kinet=False, recur_seq_len=5, n_units=5, 
                 noise=.05, bias=True, linear_bias=False, chans=[8,8], bn_moment=.01, softplus=True, 
                 inference_exp=False, img_shape=(40,50,50), ksizes=(15,11), centers=None):
        super(KineticsChannelModelFilterNoClamp, self).__init__()
...
class KineticsChannelModelFilterAmacrine(nn.Module):
    def __init__(self, bnorm=True, drop_p=0, scale_kinet=False, recur_seq_len=5, n_units=5, 
        ...
        self.kinetics = Kinetics_channel(chan=self.chans[1])
        if scale_kinet:
