...
def get_hs(model, batch_size):
    hs[0] = torch.zeros(batch_size, *model.h_shapes[0])
    hs[0][:,0] = 1
    hs[1] = deque([],maxlen=model.seq_len)
    for i in range(model.seq_len):
        hs[1].append(torch.zeros(batch_size, *model.h_shapes[1]))
    return hs
...
def train(cfg):
    ...
    for epoch in range(cfg.epoch):
        epoch_loss = 0
        hs = get_hs(model, cfg.Data.batch_size).to(device)
        ...
        if epoch%cfg.save_intrvl == 0:
            ...
            save_path = os.path.join(cfg.save_path, cfg.exp_id, 
                                     'epoch_{}_loss_{}'.format(epoch, epoch_loss))
