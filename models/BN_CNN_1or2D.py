...
from models.torch_utils import GaussianNoise, ScaleShift, Flatten, Reshape, BatchNorm1or2D
...
class BNCNN1or2D(nn.Module):
    def __init__(self, output_units=5, noise=.05, bias=True, chans=None):
        ...
        modules.append(BatchNorm1or2D(chans[0], chans[0]*36*36, eps=1e-3, momentum=.99))
        modules.append(GaussianNoise(std=noise))
        modules.append(nn.ReLU())

        modules.append(nn.Conv2d(chans[0],chans[1],kernel_size=11, bias=bias))
        modules.append(BatchNorm1or2D(chans[1], chans[1]*26*26, eps=1e-3, momentum=.99))
        modules.append(GaussianNoise(std=noise))
        modules.append(nn.ReLU())
