...
def train(hyps, model, dataGang, dataIntra, intraRF):
    ...
    intra_stim = dataIntra[0]
    intra_mempot = dataIntra[1]
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in range(num_batches):
            ...
            x = data.train_X[idxs]
            intra_x = stim.concat(intra_stim).torch().to(DEVICE)
            label = data.train_y[idxs]
            label = label.float()
            label = label.to(DEVICE)
            intra_label = intra_mempot.float().to(DEVICE)
            ...
            y_conv_2  = physio.inspect(stim, insp_keys={"sequential.6"})
			intra_y = y_conv_2(:, sidx[0], sidx[1]).float()
