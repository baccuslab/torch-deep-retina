...
if __name__=="__main__":
    ...
    norm_stats = [mean, std] # to z-score the test data with statistics of the training data
    test_data = loadexpt(dataset, cells, stim_type, 'test', temporal_depth, nskip=0, 
                                        norm_stats=norm_stats, data_path=path_to_data)

    # Compute model responses and determine pearson correlation with ganglion cell output
    batch_size = 500
    model_response = batch_compute_model_response(test_data.X, model, batch_size, 
